...
And you're telling me it's now "back"?  

Short answer: It's complicated.  

Long answer: Gapples was expensive asf. So we decided to shut it down indefinitely. And to be clear, it is still indefinitely closed. In the future, I'd like to buy myself a server to host the Gapples anarchy server once I make enough money, which would bring the expenses down to virtually $0. 
So for now, I am only hosting the lobby server (AKA Skyblock) on my own Raspberry Pi, so it's free to host. Obviously the Raspberry Pi cannot host the whole anarchy server so I had to make a decision: no Gapples at all, or the lobby server.
